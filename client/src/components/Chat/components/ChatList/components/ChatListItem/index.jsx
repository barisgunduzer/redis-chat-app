  ...
  useEffect(() => {
    if (lastMessage === undefined) {
      ...
      if (room.messages === undefined) {
        getMessages(room.id, 0, 15).then((messages) => {
          console.log('if last message', messages[messages.length - 1]);
          let message = null;
        ...
        console.log('else if last message', room.messages[0]);
        dispatch({
          type: 'set last message',
          payload: { id: room.id, lastMessage: null },
        });
      } else {
        console.log('else last message', room.messages[0]);
        dispatch({
          type: 'set last message',
          payload: {
            id: room.id,
