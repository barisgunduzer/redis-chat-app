  ...
  useEffect(() => {
    if (connected && user) {
      socket.on('user.connected', (newUser) => {
        updateUser(newUser, dispatch, `${newUser.username} bağlandı`);
      });
      // socket.on('user.typing', (newUser) => {
      //   updateUser(newUser, dispatch, `${newUser.username} yazıyor...`);
      // });
      socket.on('user.disconnected', (newUser) =>
        updateUser(newUser, dispatch, `${newUser.username} ayrıldı`),
      );
      ...
      socket.on('message', (message) => {
        ...
        dispatch({
          type: 'append message',
          payload: {
            id: message.roomId === undefined ? '0' : message.roomId,
            message,
          },
        });
      });
...
export { updateUser, useSocket, useUser };
